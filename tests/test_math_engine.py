"""
tests/test_math_engine.py — Unit tests for math_engine.py.
        ...
        for tau, expected in self.CASES:
            with self.subTest(tau=tau):
                ...
                self.assertAlmostEqual(result, expected, places=2,
                                       msg=f"τ={tau}: expected {expected}, got {result}")

    def test_formula_monotone_decreasing_below_1(self):
        """I_spam must decrease as τ increases towards 1."""
...
class TestZoneClassification(unittest.TestCase):
    ...
    def test_yellow(self):
        self.assertEqual(classify_zone(0.40), Zone.YELLOW)
...
class TestDelta(unittest.TestCase):
    def test_ahead(self):
        # 1 of 2 phases done, only 10% of time elapsed → Δ ≈ +0.40
        t_created = _dt(0)
        t_deadline = _dt(10)
        t_now = _dt(1)
    ...
    def test_behind(self):
        # 0 of 2 phases done, 90% of time elapsed → Δ ≈ -0.90
        t_created = _dt(0)
        t_deadline = _dt(10)
        t_now = _dt(9)
    ...
    def test_on_track(self):
        # 1 of 2 phases done, 50% of time elapsed → Δ = 0
        t_created = _dt(0)
        t_deadline = _dt(10)
        t_now = _dt(5)
        delta = compute_delta(1, 2, t_now, t_created, t_deadline)
...
class TestIU(unittest.TestCase):
    def test_basic(self):
        now = _dt(5)
        t_deadline = _dt(10)
        # τ at midpoint of 10-day window starting at day 0
        t_start = _dt(0)
        tau = compute_tau(now, t_start, t_deadline)  # = 0.5
        iu = compute_iu(tau, now, t_deadline)
        hours_remaining = 5 * 24  # 120h
    ...
    def test_expired_denom_clamped(self):
        ...
        t_deadline = _dt(10)
        t_start = _dt(0)
        tau = compute_tau(now, t_start, t_deadline)  # = 1.5
        iu = compute_iu(tau, now, t_deadline)
        # hours_remaining is negative, clamped to 0.1
        expected = tau / 0.1
        self.assertAlmostEqual(iu, expected, places=10)
...
class TestCooldown(unittest.TestCase):
    ...
    def test_not_elapsed(self):
        last = "2025-01-01 00:00:00"
        now = datetime(2025, 1, 1, 0, 30, 0, tzinfo=timezone.utc)  # 30 min later
        self.assertFalse(cooldown_elapsed(last, 60.0, now))  # 60-min interval not met
